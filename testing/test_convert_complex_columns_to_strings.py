...
def test_convert_complex_columns_to_strings():
    ...
    map_array = pa.array(
        [{"x": 10, "y": 20}, {}, None], type=pa.map_(pa.string(), pa.int64())
    )
    ...
    assert new_table["struct_col"][2].as_py() is None
    ...
    assert new_table["list_col"][2].as_py() is None
    ...
    assert new_table["map_col"][2].as_py() is None
