...
- The primary Python package is `avalanche_risk_mapping`, located in `src/avalanche_risk_mapping/`
- ArcGIS Pro toolboxes are in `arcgis/` and reference `avalanche_risk_mapping` for shared logic
...
- **`src/avalanche_risk_mapping/`** - Main source code for the project package
...
`src/avalanche_risk_mapping/config.py`, which exposes a singleton `ConfigNode` supporting
...
Import the pre-built singletons from `avalanche_risk_mapping.config`:

```python
from avalanche_risk_mapping.config import config, secrets, ENVIRONMENT
...
`src/avalanche_risk_mapping/config.py` for a persistent local override.
...
`pyproject.toml` declares the packages that **`avalanche_risk_mapping` itself requires to
run**. Add a dependency here when:

- A module inside `src/avalanche_risk_mapping/` contains a top-level `import` of that package
...
runtime dependency of `avalanche_risk_mapping`:
...
| Scenario | Add to |
|---|---|
| `import foo` inside `src/avalanche_risk_mapping/` | `pyproject.toml` `[project] dependencies` |
...
- Use the `@with_temp_fgdb` decorator from `avalanche_risk_mapping.utils` —
  implemented in `src/avalanche_risk_mapping/utils/_data.py` and included by default
...
```python
from avalanche_risk_mapping.utils import with_temp_fgdb
...
- Mirror the `src/avalanche_risk_mapping/` package structure in `testing/`:
    - One test file per module: `testing/test_<module_name>.py`
    - Example: `src/avalanche_risk_mapping/analysis.py` → `testing/test_analysis.py`
- Use `testing/test_avalanche_risk_mapping.py` for package-level smoke tests
...
- Run with coverage (if `pytest-cov` is installed):
  `pytest --cov=avalanche_risk_mapping --cov-report=term-missing`
...
`src/avalanche_risk_mapping/utils/_logging.py` and included by default in generated
...
Every module in `src/avalanche_risk_mapping/` should configure a logger at the top of
the file. Use `__name__` as the logger name so log output identifies the originating module:

```python
from avalanche_risk_mapping.utils import get_logger
